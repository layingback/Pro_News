...
# Required as Pro_News generated ones will be too late - after require_once(header.php)
...
$pagetitle = 'Example Module Calling on Pro_News';

# Allow for re-naming of Pro_News module
$pnsettings = $MAIN_CFG['pro_news'];
$module_name = $pnsettings['module_name'];  # must use $module_name

# Enable only if you need Pro_News messages early
...
$value = isset($_GET['c']) ? Fix_Quotes($_GET['c'],1) : '';
// echo 'cntry='.$value;
...
# NOTE: post Pro_News output will ALWAYS display on 1st page, even if Pro_News output is multi-page
	...
	$text .= 		'Pro_News CM™ is the most complete Content Module available for Dragonfly™CMS. Use for ANY and ALL Content, as a free standing module, as an adjunct to News, as a replacement for News, or for ALL 3! Only 1 copy of Pro_News ever required.';
	$text .= 	'</div>';
	$text .= '</div>';
	$text .= '<br /><br />';
...
}	# close if (is_active($module_name))
