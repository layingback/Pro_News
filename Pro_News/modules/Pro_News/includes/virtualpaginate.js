...
function virtualpaginate(config){ //config: {piececlass:, piececontainer:, pieces_per_page:, defaultpage:, wraparound:, persist}
	...
	this.paginatediv=[], this.flatviewlinks=[], this.cpspan=[], this.selectmenu=[], this.prevlinks=[], this.nextlinks=[], this.firstlinks=[], this.lastlinks=[]  	// lb add suppression for First and Last links
	...
	this.topofpage=config.topofpage || false // lb - adjust page display
}
	...
	if (!this.wraparound){
		...
		for (var i=0; i<this.nextlinks.length; i++) //add or remove "disable" class from next links depending on current page number
			virtualpaginate.setcssclass(this.nextlinks[i], "disabled", (this.currentpage==(this.pagecount-1))? "add" : "remove")
				// lb add suppression for First and Last links below
		for (var i=0; i<this.firstlinks.length; i++) //add or remove "disable" class from prev links depending on current page number
			virtualpaginate.setcssclass(this.firstlinks[i], "disabled", (this.currentpage==0)? "add" : "remove")
		for (var i=0; i<this.lastlinks.length; i++) //add or remove "disable" class from next links depending on current page number
			virtualpaginate.setcssclass(this.lastlinks[i], "disabled", (this.currentpage==(this.pagecount-1))? "add" : "remove")
	}
}
...
virtualpaginate.prototype.showpage=function(pagenumber){
	...
	for (var i=0; i<totalitems; i++){
		if (i>=showstartindex && i<=showendindex){
			...
			if (this.currentpage != 0 && !this.topofpage){ //if topofpage disabled
				if (this.pieces[i].scrollIntoView) // dd Back to Top
					this.pieces[i].scrollIntoView() // dd Back to Top
				window.scrollBy(0,-50) //lb
			} else
				window.scrollTo(0,0) //lb
		} else
	...
	if (this.cpspan.length>0){ //if <span class="pn_paginateinfo> element is present, update it with the most current info (ie: Page 3/4)
...
// paginate_build_cpinfo(cpspan)- Accepts <span class="pn_paginateinfo"> element and displays current page info (ie: Page 1/4)
// -------------------------------------------------------------------
...
virtualpaginate.prototype.paginate_build=function(divid, divpos, optnavtext){
	...
	for (var i=0; i<allspans.length; i++){
		...
		else if (allspans[i].className=="pn_paginateinfo")
...
virtualpaginate.prototype.paginate_build_selectmenu=function(paginatedropdown, divpos, anchortext){
	...
	for (var i=0; i<this.pagecount; i++){
		if (typeof anchortext[i]!="undefined") //if custom anchor text for this link exists, use anchor text as each OPTION's text
			...
			paginatedropdown.options[i]=new Option("Go to Page "+(i+1)+" of "+this.pagecount, i)
	}
...
virtualpaginate.prototype.paginate_build_regularlinks=function(paginatelinks){
	...
	for (var i=0; i<paginatelinks.length; i++){
		...
		else if (currentpagerel=="first" || paginatelinks[i].href.indexOf("first")!=-1){ //check if this is a "first" link
			if (!this.wraparound && this.currentpage==0) //if current page is first page, disable "prev" link
				virtualpaginate.setcssclass(paginatelinks[i], "disabled", "add")
			this.firstlinks.push(paginatelinks[i])
		}
		else if (currentpagerel=="last" || paginatelinks[i].href.indexOf("last")!=-1){ //check if this is a "last" link
			if (!this.wraparound && this.currentpage==this.pagecount-1) //if current page is last page, disable "next" link
				virtualpaginate.setcssclass(paginatelinks[i], "disabled", "add")
			this.lastlinks.push(paginatelinks[i])
		}
